...
https://nilhcem.com/FakeSMTP/download
...
Заполните файл email.properties в папке src/main/resources. Укажите ваш email, пароль приложения, SMTP хост и порт. Для Gmail используйте пароль приложения, а не обычный пароль.

Email (тестирование с эмулятором)

Скачайте FakeSMTP с официального сайта nilhcem.com. Запустите эмулятор на порту 2525. В файле email.properties укажите хост localhost, порт 2525 и отключите авторизацию. Все письма будут отображаться в окне эмулятора.

SMS (SMPP эмулятор)

Скачайте SMPPSim с sourceforge.net. Распакуйте архив и запустите startsmppsim.bat для Windows или startsmppsim.sh для Linux и Mac. Настройки в файле sms.properties уже указаны для стандартного эмулятора.

Telegram

Создайте бота у BotFather в Telegram. Получите токен бота. Отправьте любое сообщение вашему боту, затем выполните запрос к API для получения chat_id. Заполните файл telegram.properties своим токеном и chat_id.

Переменные окружения (рекомендуемый способ)
Вместо properties файлов можно использовать переменные окружения или файл .env. Скопируйте файл .env.example в .env и отредактируйте его со своими секретами: JWT_SECRET, DB_URL, DB_USERNAME, DB_PASSWORD.
